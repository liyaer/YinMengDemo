...
+(NSArray *)getRGBFromColor1:(UIColor *)color
{
    ...
    DLog(@"Red: %.2f  Green: %.2f  Blue: %.2f  Alpha: %.2f", red,green,blue,alpha);
...
+(NSArray *)getRGBFromColor2:(UIColor *)color
{
    ...
    DLog(@"Red: %.2f  Green: %.2f  Blue: %.2f  Alpha: %.2f", components[0],components[1],components[2],components[3]);
...
+(NSArray *)getRGBFromRgbColor1:(UIColor *)rgbColor
{
    ...
    {
        DLog(@"本次传的参数是非RGB创建的，该方法无法获取RGB值！");
    }
    return rgb;
}
...
+(NSMutableArray *)getRGBFromRgbColor2:(UIColor *)rgbColor
{
    ...
    if (RGBArr.count == 5)
    {
        ...
        DLog(@"Red: %@  Green: %.@  Blue: %.@  Alpha: %@", RGBStrValueArr[1],RGBStrValueArr[2],RGBStrValueArr[3],RGBStrValueArr[4]);
    }
    else
    {
        DLog(@"本次传的参数是非RGB创建的，该方法无法获取RGB值！");
